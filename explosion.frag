...
float fbm( vec3 p )
{
   return noise(p*.06125)*.5 + noise(p*.125)*.25 + noise(p*.25)*.125 + noise(p*.4)*.0625;
}
...
{
    float n = -mod(iTime * 0.15,-3.); // noise amount
    float iter = 2.0;
    for (int i = 0; i < 18; i++)
    {
...
{
	float b = dot(dir, org);// screen ray intercect with ball inside.
    float radius = 6.0;// sphere on (0,0,0)
	float c = dot(org, org) - pow(radius,2.);// tangent line length 
	float delta = b*b - c;
	if( delta < 0.0) 
		return false;
...
{  
	...
	vec3 rd = normalize(vec3((fragCoord.xy-0.5*iResolution.xy)/iResolution.y,1));//TODO simplest ray direction way
	vec3 ro = vec3(0., 0., -6.);
    ...
    if(RaySphereIntersect(ro, rd, min_dist, max_dist))
    {
        ...
        #endif
        {
            ...
            if(td>0.9 || d<0.12*t || t>5. || sum.a > 0.99 || t>max_dist) break;
