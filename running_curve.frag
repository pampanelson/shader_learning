...
//--------------------------------------------------------------------------------
//  1 out, 1 in...
#define HASHSCALE .0931
float Hash(float p)
{
	vec3 p3  = fract(vec3(p) * HASHSCALE);
    p3 += dot(p3, p3.yzx + 109.19);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // float direction = mod(float(iFrame),2.)>0.0?1.:-1.;
	// float pos = (direction*iTime + 2.3*uv.x) * 1.0;

    float amplitude = 2.0;
    float pos = (iTime + uv.x)*amplitude;
